...
timeIntegratedBranchRate <- BAMMtools:::timeIntegratedBranchRate
gcr <- function (ephy, node = NULL, nodetype = "include", drop.stem = FALSE, verbose = FALSE) 
{
    ...
    else if (!is.null(node[1]) & nodetype[1] == "include" & length(node) == 
        1) {
        nodeset <- getDesc(ephy, node)$desc_set
        if (drop.stem)	{
        		nodeset <- setdiff(nodeset, node)
        }
    }
    else if (!is.null(node[1]) & nodetype[1] == "exclude" & length(node) == 
        1) {
        nodeset <- setdiff(ephy$edge[, 2], getDesc(ephy, node)$desc_set)
    }
    else if (!is.null(node[1]) & length(nodetype) == length(node) & 
        length(node) > 1) {
        nodesets <- lapply(node, function(x) getDesc(ephy, x)$desc_set)
        Drop <- which(nodetype == "exclude")
        nodeset_toRemove <- unique(unlist(lapply(nodesets[Drop], 
            function(x) x)))
